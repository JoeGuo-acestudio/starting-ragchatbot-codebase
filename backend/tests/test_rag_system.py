...
2. <a href="https://example.com/lesson2" target="_blank">Neural Network Basics</a>"""
        ...
        mock_rag_system.tool_manager.get_last_sources.return_value = (
            []
        )  # Outline tool doesn't use sources
    ...
    def test_add_course_document_error(self, mock_rag_system):
        ...
        mock_print.assert_called_with(
            "Error processing course document /test/invalid.txt: Processing error"
        )
    ...
    def test_add_course_folder_new_courses(
        ...
        # Verify processing
        assert (
            mock_rag_system.document_processor.process_course_document.call_count == 2
        )
        ...
        mock_print.assert_any_call(
            f"Added new course: {course1.title} ({len(sample_course_chunks)} chunks)"
        )
        mock_print.assert_any_call(
            f"Added new course: {course2.title} ({len(sample_course_chunks)} chunks)"
        )

    def test_add_course_folder_existing_courses(
        self, mock_rag_system, sample_course, sample_course_chunks
    ):
        ...
        mock_print.assert_any_call(
            f"Course already exists: {sample_course.title} - skipping"
        )
    ...
    def test_session_workflow(self, mock_rag_system):
        ...
        # Second query
        result2, sources2 = mock_rag_system.query(
            "What about machine learning?", session_id
        )
        ...
        second_call_args = (
            mock_rag_system.ai_generator.generate_response.call_args_list[1][1]
        )
        assert (
            second_call_args["conversation_history"]
