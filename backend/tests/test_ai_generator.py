...
class TestAIGenerator:
    ...
    def test_generate_response_without_tools(self, mock_ai_generator, mock_anthropic_client, anthropic_text_response):
        ...
        assert call_args["model"] == "claude-sonnet-4-20250514"  # Updated model from ai_generator.py
    ...
    def test_generate_response_with_tools_not_used(self, mock_ai_generator, mock_anthropic_client, anthropic_text_response, mock_tool_manager):
        ...
        with patch('builtins.print') as mock_print:
            result = mock_ai_generator.generate_response("Test query", tools=tools, tool_manager=mock_tool_manager)
    ...
    def test_generate_response_with_tool_use(self, mock_ai_generator, mock_anthropic_client, mock_tool_manager):
    ...
    def test_generate_response_with_conversation_history(self, mock_ai_generator, mock_anthropic_client, anthropic_text_response):
        """Test generate_response with conversation history"""
        mock_anthropic_client.messages.create.return_value = anthropic_text_response
        
        history = "Previous conversation context"
        result = mock_ai_generator.generate_response("Test query", conversation_history=history)
    ...
    def test_handle_tool_execution(self, mock_ai_generator, mock_anthropic_client, mock_tool_manager):
        ...
        # Verify debug logging
        mock_print.assert_any_call("[DEBUG] Executing tool: search_course_content with params: {'query': 'test'}")
        ...
        assert len(final_call_args["messages"]) == 3  # user + assistant + user(tool_results)
    ...
    def test_handle_tool_execution_multiple_tools(self, mock_ai_generator, mock_anthropic_client, mock_tool_manager):
    ...
    def test_handle_tool_execution_mixed_content(self, mock_ai_generator, mock_anthropic_client, mock_tool_manager):
    ...
    def test_debug_logging_levels(self, mock_print, mock_ai_generator, mock_anthropic_client, anthropic_text_response):
    ...
    def test_sequential_tool_calling_two_rounds(self, mock_ai_generator, mock_anthropic_client, anthropic_sequential_responses, mock_tool_manager):
        """Test sequential tool calling with two full rounds"""
        # Setup sequential responses: tool -> tool -> final
        mock_anthropic_client.messages.create.side_effect = anthropic_sequential_responses
        ...
        mock_print.assert_any_call("[DEBUG] Max rounds reached, making final call without tools")

    def test_sequential_tool_calling_early_termination(self, mock_ai_generator, mock_anthropic_client, anthropic_single_round_response, mock_tool_manager):
        ...
        mock_anthropic_client.messages.create.side_effect = anthropic_single_round_response
        ...
        with patch('builtins.print') as mock_print:
            result = mock_ai_generator.generate_response(
                "Test single-round query",
                tools=tools,
                tool_manager=mock_tool_manager
            )
        
        # Verify only one tool execution
        mock_tool_manager.execute_tool.assert_called_once_with("search_course_content", query="single search")
    ...
    def test_sequential_tool_calling_max_rounds_reached(self, mock_ai_generator, mock_anthropic_client, mock_tool_manager):
        ...
        mock_anthropic_client.messages.create.side_effect = [tool_response_1, tool_response_2, final_response]
        mock_tool_manager.execute_tool.side_effect = ["Result 1", "Result 2"]
    ...
    def test_sequential_tool_calling_tool_execution_error(self, mock_ai_generator, mock_anthropic_client, anthropic_tool_use_response, mock_tool_manager):
        ...
        assert result == "I encountered an error while searching for information. Please try rephrasing your question."
        mock_print.assert_any_call("[DEBUG] Tool execution failed in round 1")

    def test_sequential_tool_calling_no_tools_immediate_response(self, mock_ai_generator, mock_anthropic_client, anthropic_text_response):
    ...
    def test_backward_compatibility_with_handle_tool_execution(self, mock_ai_generator, mock_anthropic_client, anthropic_tool_use_response, mock_tool_manager):
