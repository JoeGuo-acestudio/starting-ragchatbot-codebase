...
"""
    ...
    def generate_response(
        ...
        if tools and tool_manager:
            return self._execute_sequential_rounds(
                api_params, tool_manager, max_rounds=2
            )
    ...
    def _execute_sequential_rounds(
        ...
        while current_round < max_rounds:
            ...
            round_params = {**api_params, "messages": messages}
            ...
            if response.stop_reason == "tool_use" and tool_manager:
                ...
                if tool_results:
                    ...
                    if current_round < max_rounds:
                        ...
                        print(
                            f"[DEBUG] Max rounds reached, making final call without tools"
                        )
                ...
                print(
                    f"[DEBUG] No tool use in round {current_round}, returning response"
                )
                return response.content[0].text
        ...
        try:
            for content_block in response.content:
                if content_block.type == "tool_use":
                    print(
                        f"[DEBUG] Executing tool: {content_block.name} with params: {content_block.input}"
                    )
                    ...
                    print(
                        f"[DEBUG] Tool result length: {len(str(tool_result))} characters"
                    )
    ...
    def _make_final_call(
        self, messages: List[Dict], original_params: Dict[str, Any]
    ) -> str:
        """
